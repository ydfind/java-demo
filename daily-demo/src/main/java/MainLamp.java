...
public class MainLamp {
    ...
    public static void main(String[] args) {
        ...
        while (scanner.hasNextInt()) {
            ...
            int sLong = Integer.parseInt(scanner.next());
            int[] position = new int[num];
            for (int i = 0; i < num; i++) {
                position[i] = Integer.parseInt(scanner.next());
            }
            System.out.println(compute(position, sLong));
        }
    }

    private static String compute(int[] position, int sLong) {
        ...
        for (int i = 0; i < position.length - 1; i++) {
            int sideMax;
            sideMax = position[0] > (sLong - position[position.length - 1]) ?
                    position[0] : sLong - position[position.length - 1];
            //路两头只靠一盏灯照明，所以*2
            if (sideMax * 2 > max) {
                max = sideMax;
